...
function removeFriend(user, friendName) {
  const xhttp = new XMLHttpRequest();
  xhttp.onload = function () {
      const friendDiv = document.getElementById(`friend-${friendName}`);
      friendDiv.remove();
  }
  xhttp.open("DELETE", `http://localhost:3000/profile/delete-friend/${user}/${friendName}`);
  xhttp.send();
}